<%--File Name	: mngr_prjtHist00_p.jsp
	Description : 기업 과제이력 정보 팝업창
	author		: 임지현
	since		: 2011.02.08
	version		: 1.0
--%>
...
<%
	...
	List getPrjtHist = (List)request.getAttribute("getPrjtHist");
...
<body class="main">
	<div id="pop">
	<br />
		<h2><img src="/A_img/common/M_sub/con_tit2_ico.gif" width="14" height="9" /><%=entp_ko_nm%>(단위:천원)</h2>
		<!--테이블 시작 -->	
		<table cellpadding="0" cellspacing="0" class="table_style_4">
			...
			<tbody>
			<%if(getPrjtHist == null || getPrjtHist.isEmpty()){%>
			<tr>
			...
			<%}else{
				Map row = null;
				String cont = "";
				for(int i=0; i<getPrjtHist.size(); i++){
					row = ut.getResultNullChk((Map)getPrjtHist.get(i));
			%>
			<tr>
				<td><%=row.get("ENTP_STDT")%>~<%=row.get("ENTP_EDDT")%></td>
				<td><%=row.get("BUSS_SORT_NM")%></td>
				<td><%=row.get("CHRG_NM")%></td>
				<td><%=row.get("FNL_RSLT")%></td>
				<td><%=row.get("PRJT_NM")%></td>
				<td><%=row.get("SPRT_MY")%></td>
			</tr>
			<%
				}
			}
