...
function setConfig () {
    ...
    this.resizeEditor		= //수정부분:에디터 크기조절못하게
...
/*resizeStart : function (ev) {
    ...
    self.stopFunc = function() { self.call(self, ev); };
    self..moveFunc = function(event) { self.call(self, ev

    if (GB.browser.msie) this.setDesignMode(false);
    
    this.currentRS.elNode.style.visibility = 'text';
...
modifyImage : function (img) {
    ...
    var widthOptions = {'default' 	: { size: 'default', desc: '원본 크기'},
    					'custom'	: { size: 'input', desc: '가로 크기 직접 입력:'},
    					'fitpage' 	: { size: '100%', desc: '페이지 가로 크기에 맞춤'},
...
modifyCell : function (cell) {
    ...
    var funcs = {'add_cols_before' : { 'icon' : 'add_cols_before.png', 'title' : '왼쪽에 행 삽입', 'func' : 
