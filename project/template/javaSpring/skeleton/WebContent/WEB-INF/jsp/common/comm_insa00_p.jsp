<%--File Name	: comm_insa_p.jsp
	Description : 직원 조회 팝업 
	author		: 
	since		: 
	version		: 1.0
--%>
...
%>
<script type="text/javascript">
function setVal(num,id,nm){	
	$(opener.document).find('#entr_chrg_id_'+num).val(id);
	$(opener.document).find('#entr_chrg_nm_'+num).val(nm);
	window.close();
}
</script>
</head>
...
<body class="main">
	...
	<form name="form1" id="form1" method="post"  action="<%=request.getContextPath() %>/mngr/common/comm_insa00_p.do" >	
    ...
    <div id="search_list">
			<fieldset>
				...
				선택:<select name="gubun" id="gubun">
					...
					--><option value="dptn_nm" <%if("dptn_nm".equals(gubun)){%> selected <%}%>>부서</option>
			...
			<tbody>
		       <%if(list == null || list.isEmpty()){ %>
				...
				<%}else{ 
					...
					%>
					<tr align="center" onclick="setVal('<%=num%>', '<%=row.get("SABUN")%>', '<%=row.get("NM")%>')"  >
