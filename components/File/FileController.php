...
class FileController extends MadController {
	function indexAction() {
		...
		if ( strpos( $get->dir, PROJECT_ROOT ) === false ) {
			$get->dir = '.';
		} else {
	...
	//todo refactoring this. old, but for Project.
	function indexProjectAction() {
		if ( ! $target = $this->get->target ) {
			$target = '.';
			if ( $this->projectLog->isOpen() ) {
				$target = $this->projectLog->root;
			}
		}

		$files = new MadFile( $target );
	...
	function writeAction() {
		$this->js->addFirst("/venders/swfUploader/swfupload.js");
		$this->main->uploader = new Swfupload('test');
	}
	...
	function browserAction() {
		...
		$this->main = new FileBrowser( $get->file, $get->type );
	}
	function imageListAction() {
		$get = $this->get;
		if ( ! $get->file ) {
			$get->file = 'images';
		}
		$list = new MadFile( $get->file );
		$list->filter('^\.$|^\.\.$');
	...
	function dirListAction() {
		$get = $this->get;
		if ( ! is_dir ( $get->file ) ) {
			$get->file = dirname( $get->file );
		}
		$dirs = new MadFile( $get->file );
		$dirs->filter('^\.$|^\.\.$');
		$this->main->dirs = $dirs;
	}
	function saveDirAction() {
		...
		$result = @mkdir( $post->dirName, 0755, true );
		@chmod( $post->dirName, 0755 );
		return $result;
	}
