...
class MadL10N {
	...
	public static function setLocaleFile( $file ) {
		self::$localeFile = $file;
	}
	...
	public function isOpenFromCode( $code ) {
		$locale = clone $this->getLocales();
		return $locale->index( 'code' )->$code->open;
	}
	public function isOpen( $id ) {
		return $this->getLocale( $id )->open;
	}
	...
	private function isAdmin() {
		if ( ! $log = MadGlobals::getInstance()->log ) {
			return false;
		}
		return $log->isAdmin();
	}
	...
	// L10N support information here
	function localize() {
		// open exception
		if ( ! $this->openInfo->isEmpty() ) {
			$openInfo = $this->openInfo;
			if ( ( ! $openInfo->remoteAddr->in( $_SERVER['REMOTE_ADDR'] ) ) &&
					( ! $openInfo->openControllers->in( $globals->controllerName ) )
			   ) {
				MadHeaders::replace( $openInfo->workingUrl );
			}
		}
		$this->setTimezone();
		$session = PxParams::create('_SESSION');
		if ( $session->notrans !== 'true' ) {
			$this->setLocale();
			$getTextDomain = 'messages';
			if ( strpos( $this->sitemap->getInfo()->href, 'admin' )  ) {
				$getTextDomain = 'admin';
			}
			$this->setGettext( $getTextDomain );
		}
	}
