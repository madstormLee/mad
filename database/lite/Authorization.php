...
class Authorization {
	...
	public function __construct()
	{
		...
		if(!isset($_SESSION['_salt']) && !isset($_COOKIE['_salt']))
		{
			// create a random salt for this session if a cookie doesn't already exist for it
			$_SESSION['_salt'] = self::generateSalt(20);
		}
		else if(!isset($_SESSION['_salt']) && isset($_COOKIE['_salt']))
		{
			// session doesn't exist, but cookie does so grab it
			$_SESSION['_salt'] = $_COOKIE['_salt'];
		}

		// salted and encrypted password used for checking
		$this->system_password_encrypted = md5(SYSTEMPASSWORD."_".$_SESSION['_salt']);

		$this->authorized =
			// no password
			SYSTEMPASSWORD == ''
			// correct password stored in session
			|| isset($_SESSION['password']) && $_SESSION['password'] == $this->system_password_encrypted 
			// correct password stored in cookie
			|| isset($_COOKIE[COOKIENAME]) && isset($_COOKIE['_salt']) && md5(SYSTEMPASSWORD."_".$_COOKIE['_salt']) == $_COOKIE[COOKIENAME];
	}
	...
	public function attemptGrant($password, $remember) {
		if ($password == SYSTEMPASSWORD) {
			if ($remember) {
				...
				setcookie("_salt", $_SESSION['_salt'], $expire, null, null, null, true);
			} else {
				// user does not want to be remembered, so destroy any potential cookies
				setcookie(COOKIENAME, "", time()-86400, null, null, null, true);
				setcookie("_salt", "", time()-86400, null, null, null, true);
				unset($_COOKIE[COOKIENAME]);
				unset($_COOKIE['_salt']);
			}

			$_SESSION['password'] = $this->system_password_encrypted;
