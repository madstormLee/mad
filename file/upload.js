...
var UploadHandlers = {
	...
	uploadComplete: function(file) {
		try {
			...
			var src = '/store/admin/uploads/' + file.name;
			var image = $('<img src="' + src + '" width="300px" height="200px" />');
			target.up('dt').find('div').remove();
			target.val( src );
			target.before( image );
			target.up('dl').find('.fileName').html( file.name );
			target.up('dl').find('.fileSize').html( file.size );
			target.up('dl').find('.fileType').html( file.type );
...
var Swfuploader = {
	create: function ( id ) {
		...
		this.uploaders[id] = new SWFUpload( Swfuploader.createOptions( id ) );

		$('.swfupload .btnCancel').click( function( ev ) {
			ev.preventDefault();
			var el = $('ev.target');
			this.uploaders[id].cancelQueue();
		});
	...
	createOptions: function( id ) {
		...
		var options = {
			...
			// Flash Settings
			flash_url : form.find('.uploader').attr('href'),
...
$( function() {
	Swfuploader.create( $('form.swfupload').attr('id') );
});
